"use strict";(()=>{function c(e){let t=document.getElementById(e);if(!t)throw new Error(`missing element #${e}`);return t}async function S(e){let t=await fetch(e);if(!t.ok){let n=t.statusText;try{n=(await t.json()).detail??n}catch{}throw new Error(n)}return t.json()}function d(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function Q(e){if(e==null)return"\u2014";let t=e%1440,n=Math.floor(t/60),a=Math.round(t%60),o=n<12?"am":"pm";return`${n%12===0?12:n%12}:${String(a).padStart(2,"0")}${o}`}function ut(e){if(e==null)return"\u2014";let t=Math.floor(e/60),n=Math.round(e%60);return t?`${t}h ${String(n).padStart(2,"0")}m`:`${n}m`}function dt(e){return e>0?`+${e}`:String(e)}function pn(e,t){return e?`${t>=e?"+":""}${((t-e)/e*100).toFixed(1)}%`:t?"new":"\u2014"}var Ho="#4aa3ff",jo="#ffa23a";function Uo(e,t,n,a=96){let o=[],s=n/111320,r=n/(111320*Math.cos(e*Math.PI/180));for(let l=0;l<=a;l++){let u=l/a*2*Math.PI;o.push([t+r*Math.cos(u),e+s*Math.sin(u)])}return{type:"Feature",geometry:{type:"Polygon",coordinates:[o]},properties:{}}}function Z(e){return{type:"FeatureCollection",features:e}}function mn(e,t){return e.map(n=>({type:"Feature",geometry:{type:"Point",coordinates:[n.lon,n.lat]},properties:{...n,side:t}}))}function yn(e){e.addSource("walk",{type:"geojson",data:Z([])}),e.addSource("stops-now",{type:"geojson",data:Z([])}),e.addSource("stops-prop",{type:"geojson",data:Z([])}),e.addLayer({id:"walk-fill",type:"fill",source:"walk",paint:{"fill-color":"#8fb7ff","fill-opacity":.12}}),e.addLayer({id:"walk-line",type:"line",source:"walk",paint:{"line-color":"#8fb7ff","line-width":1.5,"line-dasharray":[2,2]}}),e.addLayer({id:"stops-prop-c",type:"circle",source:"stops-prop",paint:{"circle-radius":7,"circle-color":jo,"circle-opacity":.85,"circle-stroke-width":1,"circle-stroke-color":"#3a2a10"}}),e.addLayer({id:"stops-now-c",type:"circle",source:"stops-now",paint:{"circle-radius":4,"circle-color":Ho,"circle-stroke-width":1,"circle-stroke-color":"#0d2036"}});let t=new maplibregl.Popup({closeButton:!1,offset:10});for(let n of["stops-now-c","stops-prop-c"])e.on("mouseenter",n,()=>{e.getCanvas().style.cursor="pointer"}),e.on("mouseleave",n,()=>{e.getCanvas().style.cursor="",t.remove()}),e.on("mousemove",n,a=>{let o=a.features?.[0];if(!o)return;let s=o.properties;t.setLngLat(a.lngLat).setHTML(`<b>${s.name}</b><br>${s.side==="current"?"today":"proposed"}
    ...
    <div class="empty">
      ...
      <div class="lg-ends" style="margin-top:6px">Ground and people count the
        ...
        select. Clear the selection to count them.</div>`;function Ds(e){let{layer:t,day:n,bounds:a,unit:o,population:s,scoped:r=!1}=e;return`
    <div class="lg-ramp">
    ...
    <div class="lg-foot">A street either has a bus on it or it doesn't, so
      ...
      Locations or Surface.</div>`}function xa(e,t,n){let a=t.statuses.map(m=>m.key),o=ga(t.points,a,n.west,n.south,n.east,n.north),s=m=>t.statuses.find(v=>v.key===m)?.label??m,r=Je.reduce((m,v)=>m+(o[v]??0),0),l=Jt(t),u=t.day&&t.day!==ze,p=u?`Restricted to routes running on ${zt[t.day]} at both ends \u2014 <b>not</b> the published answer, which counts a route
    ...
    <div class="lg-foot">Can a rider reach ${d(l)} without transferring?
      ...
      South Hills would read as losing rides the Blue Line still runs.</div>`}function _a(e){return`
    ...
    <span><i class="sw-both"></i>both, same spot</span>`}var $a={locations:"Locations",riders:"Riders"};function Cs(e){let n=`${e.toLocaleString()} location${e===1?"":"s"} in view`;return`<div class="lg-foot lg-foot-riders">${e?`<b>${n}</b> ${e===1?"gains":"gain"} a bus where none stops today: no boardings to weigh. This counts what is at risk, never what is gained.`:"Boardings exist only where a bus stops today, so this counts what is at risk, never what is gained."}
    Boardings are PRT's May 2025 daily averages: unlinked trips,
    not people, and by PRT's own disclaimer up to 30% low.</div>`}function Ms(){return`<div class="lg-foot">Dots mark today's stops, plus the places the plan
    ...
    colours the pavement itself, and shows the rest.</div>`}function Pa(e,t){let{layer:n,day:a,bounds:o,weight:s,surface:r,unit:l="area",population:u,selection:p}=t,m=n.buckets.map(w=>w.key),v=n.days.indexOf(a),{west:k,south:x,east:j,north:g}=o,z=xs(n),_=p&&p.size>0?p:null,X=_?Nn(_):An(k,x,j,g),ct=Jn(n.points,v,m,X),G=s==="riders"?zn(n.points,v,m,X):null,Ao=w=>G?G.measured[w]?Math.round(G.riders[w]).toLocaleString():"\u2014":ct[w].toLocaleString(),No=_?`at ${_.size.toLocaleString()} selected stop${_.size===1?"":"s"}`:"in view",Bo=G?`<b>${Math.round(z.reduce((w,pe)=>w+G.riders[pe.key],0)).toLocaleString()}</b> daily boardings ${No}`:_?`<b>${z.reduce((w,pe)=>w+ct[pe.key],0).toLocaleString()}</b>
    ...
    <div class="lg-foot">Fill is signed: red where a place's own trips fall,
      ...
      it.</div>`}function io({selected:e,fill:t,day:n,boundaries:a,unchanged:o}){if(t==="service")return rr(e,o??null,n,a??null);let s=t==="lost"?"lose all buses":"gain a bus",r=B.filter(l=>l.opacity>0).map(l=>`
    <div class="lg-row lg-static">
