"use strict";(()=>{function c(e){let t=document.getElementById(e);if(!t)throw new Error(`missing element #${e}`);return t}async function S(e){let t=await fetch(e);if(!t.ok){let n=t.statusText;try{n=(await t.json()).detail??n}catch{}throw new Error(n)}return t.json()}function d(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function Q(e){if(e==null)return"\u2014";let t=e%1440,n=Math.floor(t/60),a=Math.round(t%60),o=n<12?"am":"pm";return`${n%12===0?12:n%12}:${String(a).padStart(2,"0")}${o}`}function ut(e){if(e==null)return"\u2014";let t=Math.floor(e/60),n=Math.round(e%60);return t?`${t}h ${String(n).padStart(2,"0")}m`:`${n}m`}function dt(e){return e>0?`+${e}`:String(e)}function pn(e,t){return e?`${t>=e?"+":""}${((t-e)/e*100).toFixed(1)}%`:t?"new":"\u2014"}var Ho="#4aa3ff",Bo="#ffa23a";function jo(e,t,n,a=96){let o=[],s=n/111320,r=n/(111320*Math.cos(e*Math.PI/180));for(let l=0;l<=a;l++){let u=l/a*2*Math.PI;o.push([t+r*Math.cos(u),e+s*Math.sin(u)])}return{type:"Feature",geometry:{type:"Polygon",coordinates:[o]},properties:{}}}function Z(e){return{type:"FeatureCollection",features:e}}function mn(e,t){return e.map(n=>({type:"Feature",geometry:{type:"Point",coordinates:[n.lon,n.lat]},properties:{...n,side:t}}))}function yn(e){e.addSource("walk",{type:"geojson",data:Z([])}),e.addSource("stops-now",{type:"geojson",data:Z([])}),e.addSource("stops-prop",{type:"geojson",data:Z([])}),e.addLayer({id:"walk-fill",type:"fill",source:"walk",paint:{"fill-color":"#8fb7ff","fill-opacity":.12}}),e.addLayer({id:"walk-line",type:"line",source:"walk",paint:{"line-color":"#8fb7ff","line-width":1.5,"line-dasharray":[2,2]}}),e.addLayer({id:"stops-prop-c",type:"circle",source:"stops-prop",paint:{"circle-radius":7,"circle-color":Bo,"circle-opacity":.85,"circle-stroke-width":1,"circle-stroke-color":"#3a2a10"}}),e.addLayer({id:"stops-now-c",type:"circle",source:"stops-now",paint:{"circle-radius":4,"circle-color":Ho,"circle-stroke-width":1,"circle-stroke-color":"#0d2036"}});let t=new maplibregl.Popup({closeButton:!1,offset:10});for(let n of["stops-now-c","stops-prop-c"])e.on("mouseenter",n,()=>{e.getCanvas().style.cursor="pointer"}),e.on("mouseleave",n,()=>{e.getCanvas().style.cursor="",t.remove()}),e.on("mousemove",n,a=>{let o=a.features?.[0];if(!o)return;let s=o.properties;t.setLngLat(a.lngLat).setHTML(`<b>${s.name}</b><br>${s.side==="current"?"today":"proposed"}
    ...
    <div class="empty">
      ...
      </tr>`}).join("")}function Tn(e){return e.length?e.map(t=>`<span class="route">${d(t)}</span>`).join(" "):'<span class="muted">none</span>'}function kn(e){return e.first==null?'<span class="muted">no service</span>':`${Q(e.first)}\u2013${Q(e.last)}`}function xn(e){let t=Object.values(e.headways).map(n=>n.median).filter(n=>n!=null);return t.length?Math.min(...t):null}var Go={here:"you are here",keeps:"keeps a one-seat ride",gains:"gains a one-seat ride",loses:"loses its one-seat ride",none:"no one-seat ride either way"},Ko={weekday:"a weekday",saturday:"a Saturday",sunday:"a Sunday"};function Yo(e,t="any"){return e.length?`
    <div class="oneseat">
      ...
      <div class="os-row">
        <div class="os-head">
          ...
          <span class="os-status ${d(a.status)}">${Go[a.status]??a.status}</span>
        </div>
        <div class="os-routes">${o}</div>
      </div>`}).join("")}
      <p class="note">${t==="any"?`One route serving both ends, on any calendar \u2014 the published
             measure.`:`Only routes running on ${Ko[t]??t} \u2014 not the
      ...
      <div class="rside"><span class="rlab">proposed</span>
        ${On(t,n,"prop")}</div>
    </div>`}function On(e,t,n){return e.length?e.map(a=>`<span class="route ${t.has(a)?"both":`only-${n}`}">${d(a)}</span>`).join(" "):'<span class="muted">none</span>'}var ht=/\s*\(([^,()]+),\s*[A-Za-z]{2}\)\s*$/,Vo="Allegheny";function ye(e){let t=e.place?.muni?.trim()??"",n=ht.exec(t)?.[1],a=n===Vo?t.replace(ht,""):n?`${t.replace(ht,"")} (${n})`:t;return e.place?.hood||a||"this location"}function bt(e){return e==="weekday"?"weekday":e}function En(e,t){let n=e.current.days[t],a=e.proposed.days[t];return`${n.trips} \u2192 ${a.trips} buses per ${bt(t)}`}function Wo(e,t){if(!e)return"";let n=e.measured+e.unmeasured,a=e.unmeasured?`<div class="muted">${e.unmeasured} of the ${n} stops
       ...
       <span class="muted">on an average ${bt(t)}, today only</span>`}${a}</dd>`}function qo(e){return!e||e.total==null?"":`<p class="note">Today's stops only \u2014 the plan's gains have no riders
    to weigh. PRT calls these unofficial totals that may understate ridership
    by up to 30%.${Oe("boardings")}</p>`}function Xo(e){if(!e)return"";let t=d(e.place),n=e.lost||e.gained?`<p class="people-n"><b>${Math.round(e.lost).toLocaleString()}</b>
    ...
    <div class="tiers">${Jo(a.hourly,o.hourly)}</div>
    ...
    ${vt(e,ft,Yo(e.oneseat??[],e.oneseat_day??"any"))}`}var Qo={keeps:"Keeps its one-seat ride",gains:"Gains a one-seat ride",loses:"Loses its one-seat ride",none:"No one-seat ride, before or after",here:"You are already there"},Zo={keeps:"Some single route serves both ends today and still does under the plan.",gains:"No single route serves both ends today; one does under the plan.",loses:"A single route serves both ends today; none does under the plan.",none:"Reaching it means changing bus on both networks \u2014 for Oakland that is most of the county, before and after.",here:"This point is inside the destination, so no one-seat ride is needed to reach it."},es={weekday:"a weekday",saturday:"a Saturday",sunday:"a Sunday"};function ts(e,t){let n=e.oneseat??[];return"lat"in t?n.find(a=>a.key===null)??null:n.find(a=>a.key===t.key)??null}function wt(e,t){return t.length?`<div class="rrow"><span class="rlab">${e}</span>${Tn(t)}</div>`:""}function ns(e){let t=wt("kept",e.kept)+wt("lost",e.lost)+wt("gained",e.gained);if(!t)return"";let n=e.lost.length&&e.gained.length?`Renumbering is not replacement, so a route in <b>lost</b> beside a
    ...
    </div>`}function as(e){return`
    <div class="routes">
      <h3>Routes reaching it from here</h3>
      ${De(e.current,e.proposed)}
    </div>`}function os(e,t){let n=(e.oneseat??[]).filter(o=>o!==t&&o.key!==null);return n.length?`
    <div class="oneseat">
    ...
    <button class="os-other" data-goto-dest="${d(o.key)}">
      ...
      <span class="os-status ${d(o.status)}">${ss[o.status]}</span>
    </button>`).join("")}</div>
      <p class="note">Click one to measure the whole map to it instead.</p>
    </div>`:""}var ss={here:"you are here",keeps:"keeps",gains:"gains",loses:"loses",none:"no ride either way"};function rs(e){return e==="any"?`Counted on any calendar, which is the published measure \u2014 no day type
       ...
       on any calendar.`}function Mn(e,t,n){let a=ts(e,t);if(!a)return"";let o=e.oneseat_day??"any",s=a.status==="here"?"":ns(a)+as(a);return`
    <div class="place-head">
    ...
    <div class="os-verdict ${d(a.status)}">${Qo[a.status]}</div>
    <p class="note">${Zo[a.status]} ${rs(o)}</p>
    ...
    <div class="empty">
      ...
      <div class="lg-ends" style="margin-top:4px">of ground in view, not of people</div>`}function _s(e,t,n){let a='<div class="lg-ends" style="margin-top:4px">where people live in view \u2014 2020 census, counted at home, not where they board</div>';if(!n)return`<div class="lg-area"><span class="muted">loading\u2026</span></div>${a}`;let o=sa(n.cells,n.days.indexOf(e),t.west,t.south,t.east,t.north,n.origin),s=r=>Math.round(r).toLocaleString();return`
      <div class="lg-area">
      ...
      <div class="lg-ends" style="margin-top:6px">Ground and people are
        ...
        has no stop to select. Clear the selection to count them.</div>`;function Os(e){let{layer:t,day:n,bounds:a,unit:o,population:s,scoped:r=!1}=e;return`
    <div class="lg-ramp">
    ...
    that may understate ridership by up to 30%.</div>`}function Pa(e,t){let{layer:n,day:a,bounds:o,weight:s,surface:r,unit:l="area",population:u,selection:p}=t,m=n.buckets.map(w=>w.key),v=n.days.indexOf(a),{west:k,south:x,east:j,north:g}=o,z=ks(n),_=p&&p.size>0?p:null,X=_?An(_):Nn(k,x,j,g),ct=Jn(n.points,v,m,X),G=s==="riders"?zn(n.points,v,m,X):null,Fo=w=>G?G.measured[w]?Math.round(G.riders[w]).toLocaleString():"\u2014":ct[w].toLocaleString(),No=_?`at ${_.size.toLocaleString()} selected stop${_.size===1?"":"s"}`:"in view",Ao=G?`<b>${Math.round(z.reduce((w,pe)=>w+G.riders[pe.key],0)).toLocaleString()}</b> daily boardings ${No}`:_?`<b>${z.reduce((w,pe)=>w+ct[pe.key],0).toLocaleString()}</b>
    ...
    ${z.map(w=>`
      <button class="lg-row ${Lt(w.key)?"off":""}" data-bucket="${d(w.key)}"
        ...
        <span class="lg-n">${Fo(w.key)}</span>
      </button>`).join("")}
    ${r?Os({layer:r,day:a,bounds:o,unit:l,population:u,scoped:!!_}):""}
    ${G?Es(G.unmeasured):`
    ...
    </table>`}function Hs(e){let t=e.radii.strict,n=t.transfer_walk_m,a=e.sign_flips?`<p class="js-flip"><b>These two disagree about which network is
    ...
    <p class="lg-foot">Rides follow the street the bus drives; dashed sections
      ...
      panel answers at a stricter one too.</p>`}var qe="places",Ya="places-points",qt="places-boundaries",C="places-fill",oe="lost",Bs=100,js={lost:"share_lost",gained:"share_gained"};function I(e,t){return`service_${e}_${t}`}var Va={weekday:"a weekday",saturday:"a Saturday",sunday:"a Sunday"},Us="Every one of Allegheny County's 1,238,177 residents is in a named place: places are assigned by boundary, not by distance to a labelled stop, so nobody here goes unnamed. Every figure is Allegheny-only and day-free \u2014 losing all buses on any day of the week \u2014 so it does not move with the toolbar's day switch. A place with under 100 residents is shown without a share: a denominator that small cannot carry one.",Y={lost:R,gained:T},Ve=null,U=null,ae=null,Wa=!1,We=null;function Xt(){return Ve}function qa(){return U}function Xa(){return We}function Qt(){return ae}function ve(){return Wa}function Is(e,t){let n=[...e];return t==="count"?n.sort((a,o)=>o.residents_lost-a.residents_lost):n.sort((a,o)=>(o.share_lost??-1)-(a.share_lost??-1))}function Js(e){return e.residents_gained>e.residents_lost?"gained":"lost"}function zs(e){return Math.max(e.residents_lost,e.residents_gained)}var Ga=4,Gs=16,Ks=1e3;function Ys(e){let t=Math.min(1,Math.sqrt(e/Ks));return Ga+t*(Gs-Ga)}function Vs(e){return{type:"FeatureCollection",features:e.changed.map(t=>({type:"Feature",geometry:{type:"Point",coordinates:[t.lon,t.lat]},properties:{geoid:t.geoid,klass:Js(t),residents_lost:t.residents_lost,residents_gained:t.residents_gained,radius:Ys(zs(t))}}))}}function Ws(){return["match",["get","klass"],"lost",Y.lost,"gained",Y.gained,Y.lost]}function qs(){return["interpolate",["linear"],["zoom"],9,["*",["get","radius"],.5],12,["get","radius"],16,["*",["get","radius"],1.6]]}var H=[{max:0,label:"No loss, or too few residents to share",opacity:0},{max:.05,label:"Up to 5%",opacity:.15},{max:.15,label:"5\u201315%",opacity:.35},{max:.3,label:"15\u201330%",opacity:.55},{max:null,label:"Over 30%",opacity:.8}];var B=[{max:10,opacity:0},{max:30,opacity:.3},{max:60,opacity:.55},{max:1/0,opacity:.8}];function Qa(e,t){return e==="service"?["step",["abs",["coalesce",["get",I(t,"pct")],0]],B[0].opacity,B[0].max,B[1].opacity,B[1].max,B[2].opacity,B[2].max,B[3].opacity]:["step",["coalesce",["get",js[e]],0],H[0].opacity,Number.EPSILON,H[1].opacity,H[1].max,H[2].opacity,H[2].max,H[3].opacity,H[3].max,H[4].opacity]}function Za(e,t){return e==="service"?["case",[">=",["coalesce",["get",I(t,"pct")],0],0],T,R]:Y[e]}function Xs(e,t){let n=I(t,"now"),a=I(t,"proposed");return e.features.filter(o=>o.properties[n]===0&&o.properties[a]>0).map(o=>o.properties.place)}var Qs=3;function Zs(e){if(e.length===0)return"";let t=e.slice(0,Qs),n=e.length-t.length,a=t.length<=1?t.join(""):`${t.slice(0,-1).join(", ")} and ${t[t.length-1]}`,o=n>0?`${a} (and ${n} more)`:a;return e.length===1?`1 place gets its first bus and cannot be shown as a percentage: ${o}.`:`${e.length} places get their first bus and cannot be shown as a percentage: ${o}.`}function eo(e,t){e.addSource(qt,{type:"geojson",data:{type:"FeatureCollection",features:[]}}),e.addLayer({id:C,type:"fill",source:qt,layout:{visibility:"none"},paint:{"fill-color":Za(oe),"fill-opacity":Qa(oe),"fill-outline-color":"rgba(255,255,255,.25)"}},t),e.addSource(qe,{type:"geojson",data:{type:"FeatureCollection",features:[]}}),e.addLayer({id:Ya,type:"circle",source:qe,layout:{visibility:"none"},paint:{"circle-color":Ws(),"circle-radius":qs(),"circle-opacity":.85,"circle-stroke-color":"rgba(255,255,255,.9)","circle-stroke-width":["interpolate",["linear"],["zoom"],9,.4,12,.9,16,1.5]}},t)}function Xe(e,t,n){e.setPaintProperty(C,"fill-color",Za(t,n)),e.setPaintProperty(C,"fill-opacity",Qa(t,n))}async function to(){return Ve||(Ve=await S("/api/places")),Ve}async function no(e){return ae||(ae=await S("/api/boundaries"),e.getSource(qt).setData(ae)),ae}function er(e,t){let n=e?.features.find(a=>a.properties.key===t);return n&&n.properties.changed_block_groups===0?n.properties.place:null}async function ao(e,t){let n=er(ae,t);if(n)return U=null,We=n,e.getSource(qe)?.setData({type:"FeatureCollection",features:[]}),null;try{U=await S(`/api/places/${encodeURIComponent(t)}`)}catch{return U=null,We=null,null}return We=null,e.getSource(qe).setData(Vs(U)),e.flyTo({center:[U.lon,U.lat],zoom:13}),U}function oo(e,t){Wa=t,e.setLayoutProperty(Ya,"visibility",t?"visible":"none"),e.setLayoutProperty(C,"visibility",t?"visible":"none")}function tr(e,t){let n=e.share_lost==null?`<span class="place-share muted" title="Too few residents here to put a share on: this place's measured population is under 100.">\u2014</span>`:`<span class="place-share muted">${(e.share_lost*100).toFixed(1)}% of the place</span>`;return`
    ...
    </button>`}var nr="Unlike the two residents readings above, this one moves with the toolbar's day switch: it is asking about the plan's actual weekday, Saturday or Sunday service, not residents' day-free losses and gains.";function so(e,t,n,a){let o=Is(e,t).map(s=>tr(s,s.key===n)).join("");return`
    <div class="place-head">
      <h2>Places</h2>
      <div class="muted">${e.length.toLocaleString()} named places the plan changes</div>
    </div>
    <p class="note">${Us}</p>
    ${a==="service"?`<p class="note">${nr}</p>`:""}
    ...
    <div class="place-list">${o}</div>`}function ro(e,t){return e?`<div class="lg-head"><b>${d(e.place)}</b>
      ...
      <div class="lg-foot muted">No block group in it loses or gains all
        service. Shaded places are the ones with something to show.</div>`:'<div class="lg-head">Click a place to see its changed block groups</div>'}function ar(e,t){return e.max===1/0?`Over ${t}%`:`${t}\u2013${e.max}%`}function or(e,t,n,a){let o=B.map((u,p)=>({band:u,prevMax:p===0?0:B[p-1].max})).filter(({band:u})=>u.opacity>0).flatMap(({band:u,prevMax:p})=>{let m=ar(u,p);return[`<div class="lg-row lg-static">
          ...
          <span class="lg-lab">${d(m)} more trips</span></div>`]}).join(""),s=a?Xs(a,n):[],r=Zs(s),l=r?`<div class="lg-foot">${d(r)}</div>`:"";return`
    ${ro(e,t)}
    ...
    <div class="lg-foot">Fill is signed: red where a place's own trips fall,
      ...
      it.</div>`}function io({selected:e,fill:t,day:n,boundaries:a,unchanged:o}){if(t==="service")return or(e,o??null,n,a??null);let s=t==="lost"?"lose all buses":"gain a bus",r=H.filter(l=>l.opacity>0).map(l=>`
    <div class="lg-row lg-static">
    ...
    <div class="lg-foot">Fill is coloured by SHARE, not by count of residents
      ...
      it; size is the larger of a block group's losses or gains.</div>`}function sr(e,t){let n=e[I(t,"now")],a=e[I(t,"proposed")],o=e[I(t,"pct")],s=e[I(t,"rail_proposed")],r=Va[t];if(a===0&&n>0)return`Loses all buses on ${r} (${n} \u2192 0 trips)${s?"; the T still calls here":""}.`;if(n===0&&a>0)return`Gets its first bus on ${r} (0 \u2192 ${a} trips).`;let l=o==null?"\u2014":`${o>0?"+":""}${o.toFixed(1)}%`;return`${n} \u2192 ${a} trips on ${r} (${l}).`}function lo(e,t,n){if(t==="service")return`<b>${d(e.place)}</b> <span class="muted">\xB7 ${d(e.kind)}</span><br>
      ${sr(e,n)}`;let a=Math.round(e.residents_total??0).toLocaleString();if(e.changed_block_groups===0)return`<b>${d(e.place)}</b> <span class="muted">\xB7 ${d(e.kind)}</span><br>
    ...
    ${r.join("<br>")}<br>
    <span class="muted">${a} residents total \xB7 ${e.changed_block_groups} block group${e.changed_block_groups===1?"":"s"} changed</span>`}function Ka(e,t,n){let a=Math.round(t).toLocaleString(),o=n==null?`share withheld \u2014 under ${Bs} residents`:`${(n*100).toFixed(1)}%`;return`${a} ${e} (${o})`}var Zt=" \xB7 ",en={dots:"Locations",surface:"Surface",both:"Locations + surface",corridors:"Streets",oneseat:"One-seat ride",journey:"Travel time",places:"Places"},co=Object.keys(en);function uo(e){return en[e]??e}var rr={weekday:"a weekday",saturday:"a Saturday",sunday:"a Sunday"},ir=["oneseat","journey"];function lr(e){return e!=="journey"}function cr(e){let t=[en[e.view]??e.view];return e.view==="places"?t[0]:(ir.includes(e.view)&&(t[0]+=` to ${e.destination}`),t.push(e.view==="oneseat"&&!e.oneSeatRestricted?"any day":rr[e.day]),lr(e.view)&&t.push(`${e.radius} m walk`),t.join(Zt))}function po(e){let[t,...n]=cr(e).split(Zt);return`<b>${d(t)}</b>${n.map(a=>Zt+d(a)).join("")}`}var h={view:"view",day:"day",radius:"radius",oneSeatDay:"oneseatday",dest:"dest",weight:"weight",surfaceUnit:"surfaceunit",at:"at",camera:"map",place:"place",placeFill:"placefill",selection:"sel"},ur=/^[cp]:[\w.:-]{1,32}$/,Qe={any:"any",selected:"selected"},dr="pin",mo=5;function go(e){try{return e.self!==e.top}catch{return!0}}function ho(e){let t=new URLSearchParams;return t.set(h.view,e.view),t.set(h.day,e.day),t.set(h.radius,String(e.radius)),t.set(h.oneSeatDay,e.oneSeatRestricted?Qe.selected:Qe.any),t.set(h.dest,"key"in e.dest?e.dest.key:tn(e.dest)),e.weight==="riders"&&t.set(h.weight,e.weight),e.surfaceUnit==="people"&&t.set(h.surfaceUnit,e.surfaceUnit),e.at&&t.set(h.at,tn(e.at)),e.camera&&t.set(h.camera,`${tn(e.camera)},${e.camera.zoom.toFixed(2)}`),e.place&&t.set(h.place,e.place),e.placeFill!==oe&&t.set(h.placeFill,e.placeFill),e.selection.length&&t.set(h.selection,e.selection.join(",")),`?${t}`}function fo(e){let t=new URLSearchParams(e),n={},a=t.get(h.view);a&&co.includes(a)&&(n.view=a);let o=t.get(h.day);o&&O.includes(o)&&(n.day=o);let s=Number(t.get(h.radius));t.has(h.radius)&&Number.isFinite(s)&&s>0&&(n.radius=s),t.get(h.weight)==="riders"?n.weight="riders":t.get(h.weight)==="locations"&&(n.weight="locations"),t.get(h.surfaceUnit)==="people"?n.surfaceUnit="people":t.get(h.surfaceUnit)==="area"&&(n.surfaceUnit="area");let r=t.get(h.oneSeatDay);r===Qe.selected?n.oneSeatRestricted=!0:r===Qe.any&&(n.oneSeatRestricted=!1);let l=t.get(h.dest);if(l&&l!==dr){let x=yo(l);x?n.dest=x:l.includes(",")||(n.dest={key:l})}let u=yo(t.get(h.at));u&&(n.at=u);let p=pr(t.get(h.camera));p&&(n.camera=p);let m=t.get(h.place);m&&(n.place=m);let v=t.get(h.selection);v!==null&&(n.selection=v.split(",").filter(x=>ur.test(x)));let k=t.get(h.placeFill);return(k==="lost"||k==="gained"||k==="service")&&(n.placeFill=k),n}function tn(e){return`${e.lat.toFixed(mo)},${e.lon.toFixed(mo)}`}function yo(e){let t=bo(e,2);return t?{lat:t[0],lon:t[1]}:null}function pr(e){let t=bo(e,3);return t?{lat:t[0],lon:t[1],zoom:t[2]}:null}function bo(e,t){if(!e)return null;let n=e.split(",").map(Number);return n.length!==t||!n.every(Number.isFinite)?null:n}var nn="embed";var mr=["1","true","yes"];function vo(e){let t=new URLSearchParams(e).get(nn);return t!==null&&mr.includes(t.toLowerCase())}function wo(e){let t=new URLSearchParams(e);return t.set(nn,"1"),`?${t}`}function So(e){let t=new URLSearchParams(e);t.delete(nn);let n=String(t);return n?`?${n}`:""}function Lo(e){return(e?`Full answer for ${e}`:"Open the full map")+" \u2197"}var J=["peek","half","full"],yr=192,gr=.3,hr=.55,fr=.9,br=.6,vr=.45;function Ze(e,t){return e==="peek"?Math.min(yr,t*gr):e==="half"?t*hr:t*fr}function wr(e,t,n=0){let a=J.map(s=>Math.abs(Ze(s,t)-e)),o=a.indexOf(Math.min(...a));return Math.abs(n)>br&&(o=Math.max(0,Math.min(J.length-1,o+(n>0?1:-1)))),J[o]}function $o(e){return J[(J.indexOf(e)+1)%J.length]}function Sr(e,t){return Math.min(e,t*vr)}function se(){return getComputedStyle(document.documentElement).getPropertyValue("--compact").trim()==="1"}function an(e){let t=null,n=()=>{let a=se();a!==t&&(t=a,e(a))};return window.addEventListener("resize",n),n(),n}var Lr=8,$r=400;function ko(e){let t=c("side"),n=c("sheet-handle"),a="peek",o=!1,s=0,r=0,l=0,u={y:0,t:0};function p(){return window.innerHeight}function m(g){t.style.height=`${g}px`,e.onMove(g,Sr(g,p()))}function v(g){a=g,t.dataset.snap=g,m(Ze(g,p()))}n.addEventListener("pointerdown",g=>{se()&&(o=!0,s=g.clientY,r=t.getBoundingClientRect().height,l=g.timeStamp,u={y:g.clientY,t:g.timeStamp},t.classList.add("dragging"),n.setPointerCapture(g.pointerId))}),n.addEventListener("pointermove",g=>{if(!o)return;let z=r+(s-g.clientY),_=Ze("peek",p()),X=Ze("full",p());m(Math.max(_,Math.min(X,z))),u={y:g.clientY,t:g.timeStamp}});function k(g){if(!o)return;if(o=!1,t.classList.remove("dragging"),!(Math.abs(g.clientY-s)>Lr)&&g.timeStamp-l<$r){v($o(a));return}let _=g.timeStamp-u.t,X=_>0?(u.y-g.clientY)/_:0;v(wr(t.getBoundingClientRect().height,p(),X))}n.addEventListener("pointerup",k),n.addEventListener("pointercancel",k),n.addEventListener("keydown",g=>{g.key!=="Enter"&&g.key!==" "||(g.preventDefault(),se()&&v($o(a)))});let x=an(e.onLayoutChange);function j(){if(x(),!se()){t.style.height="",t.removeAttribute("data-snap"),e.onMove(0,0);return}v(a)}return window.addEventListener("resize",j),j(),{at:()=>se()?a:"full",atLeast(g){se()&&J.indexOf(g)>J.indexOf(a)&&v(g)}}}var kr=[-79.9959,40.4406],xr=12,_r="#e2574c",D={radius:"data-radius",day:"data-day",oneSeatDay:"data-oneseat-day",view:"data-view",dest:"data-dest",placeFill:"data-place-fill"},Se=fo(location.search),$e=vo(location.search);$e&&c("app").classList.add("embed");var Pr={at:()=>"full",atLeast(){}},_o=null,P=400,we=null,f=null,ie=null,q=0,$={key:"downtown"},V=null,Po=!1,ue=!1,rt="locations",de="area",Oo="count",ot=null,M=oe,N=!1,y="dots",Do,rn=[],i=new maplibregl.Map({container:"map",style:"https://tiles.openfreemap.org/styles/positron",center:Se.camera?[Se.camera.lon,Se.camera.lat]:kr,zoom:Se.camera?.zoom??xr,cooperativeGestures:go(window),attributionControl:{compact:!0}});i.addControl(new maplibregl.NavigationControl,"top-right");i.on("load",()=>{yn(i),Kn(i),aa(i,"change-dots"),ca(i,"change-dots"),ha(i,"walk-fill"),Ba(i),eo(i,"change-dots"),F(),i.on("click",t=>{if(N)return;if(Po){Le({lat:t.lngLat.lat,lon:t.lngLat.lng});return}if(y==="places"){let s=i.queryRenderedFeatures(t.point,{layers:[C]})[0];s&&tt(s.properties.key);return}let n=["change-dots","oneseat-dots"].filter(s=>i.getLayoutProperty(s,"visibility")!=="none"),a=i.queryRenderedFeatures(t.point,{layers:n})[0],o=a?a.geometry.coordinates:[t.lngLat.lng,t.lngLat.lat];dn(o[1],o[0])}),i.on("mouseenter",C,()=>{i.getCanvas().style.cursor="pointer"}),i.on("mouseleave",C,()=>{i.getCanvas().style.cursor=""});let e=new maplibregl.Popup({closeButton:!1,offset:8});i.on("mouseenter","change-dots",()=>{i.getCanvas().style.cursor="pointer"}),i.on("mouseleave","change-dots",()=>{i.getCanvas().style.cursor="",e.remove()}),i.on("mousemove","change-dots",t=>{let n=t.features?.[0],a=St();!n||!a||e.setLngLat(n.geometry.coordinates).setHTML(Wn(n.properties,L(),a.buckets)).addTo(i)}),i.on("mouseenter","oneseat-dots",()=>{i.getCanvas().style.cursor="pointer"}),i.on("mouseleave","oneseat-dots",()=>{i.getCanvas().style.cursor="",e.remove()}),i.on("mousemove","oneseat-dots",t=>{let n=t.features?.[0],a=ne();!n||!a||e.setLngLat(n.geometry.coordinates).setHTML(Sa(n.properties,a)).addTo(i)}),i.on("mouseleave",C,()=>e.remove()),i.on("mousemove",C,t=>{let n=t.features?.[0];n&&e.setLngLat(t.lngLat).setHTML(lo(n.properties,M,L())).addTo(i)}),Jr(),i.on("moveend",()=>{let t=i.getCenter();_o={lat:t.lat,lon:t.lng,zoom:i.getZoom()},b(),A()}),re(D.radius,t=>{P=Number(t.dataset.radius),_t(i,P,L()).then(b),Ne()&&Et(i,P,L()).then(b),Ae()&&Ft(P).then(b),ne()&&nt(),f&&le(f.lat,f.lon)}),re(D.day,t=>{let n=t.dataset.day;Dn(n),y!=="journey"&&F(),Pt(i,n),Ct(i,n),y==="journey"&&f&&ln(f.lat,f.lon),Ue()&&ua(i,n).then(b),ue&&ne()&&(nt(),f&&le(f.lat,f.lon)),ve()&&M==="service"&&Xe(i,M,n),b()}),re(D.oneSeatDay,t=>{ue=t.dataset.oneseatDay==="selected",sn(),nt(),f&&le(f.lat,f.lon)}),re(D.view,t=>{let n=y;y=t.dataset.view,i.setLayoutProperty("change-dots","visibility",y==="dots"||y==="both"?"visible":"none"),Cr(y==="surface"||y==="both"),Fr(y==="corridors"),Br(y==="oneseat"),Hr(y==="journey",n==="journey"),Nr(y==="places"),y!=="journey"&&n!=="journey"&&(y==="oneseat"||n==="oneseat")&&F({scrollToTop:!0}),Ar(y!=="corridors"&&y!=="journey"&&y!=="places");let a=y==="oneseat"||y==="journey";c("dest-controls").classList.toggle("hidden",!a),c("oneseat-day-controls").classList.toggle("hidden",y!=="oneseat"),c("place-fill-controls").classList.toggle("hidden",y!=="places"),st()||xo(!1),ce(),sn(),a||at(!1),To()}),re(D.dest,t=>{let n=t.dataset.dest;if(n==="pin"){at(!0);return}at(!1),Le({key:n})}),re(D.placeFill,t=>{M=t.dataset.placeFill,ve()&&Xe(i,M,L()),F(),b(),sn()}),c("legend").addEventListener("click",t=>{let n=t.target.closest("[data-weight]");if(n){rt=n.dataset.weight,b(),A();return}let a=t.target.closest("[data-surface-unit]");if(a){de=a.dataset.surfaceUnit,Mr(de),A();return}let o=t.target.closest("[data-bucket]");o&&(Yn(i,o.dataset.bucket,L()),b())}),c("legend-reset").addEventListener("click",()=>{Vn(i,L()),b()}),c("legend-select").addEventListener("click",()=>xo(!N)),c("legend-clear").addEventListener("click",()=>{kt(i),ce(),b(),A()}),c("legend-collapse").addEventListener("click",()=>{on(!c("legend-box").classList.contains("collapsed"))}),c("panel").addEventListener("click",t=>{let n=t.target.closest("[data-goto-dest]");n&&Le({key:n.dataset.gotoDest});let a=t.target.closest("[data-caveat]");a&&Kr(a.dataset.caveat);let o=t.target.closest("[data-select-place]");o&&tt(o.dataset.selectPlace);let s=t.target.closest("[data-sort-places]");s&&(Oo=s.dataset.sortPlaces,F());let r=t.target.closest("[data-goto-place]");r&&(y!=="places"&&W(D.view,"places"),tt(r.dataset.gotoPlace))}),c("side-toggle").addEventListener("click",Tr),$e&&an(on),Do=$e?Pr:ko({onMove(t,n){document.documentElement.style.setProperty("--sheet-h",`${t}px`),i.setPadding({top:0,right:0,bottom:n,left:0})},onLayoutChange:on}),Dr(),lt(),ce(),it(),Or(Se)||_t(i,P,L()).then(b),Gr(),zr()});function re(e,t){let n=`[${e}]`;document.querySelectorAll(n).forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(n).forEach(o=>o.classList.toggle("active",o===a)),t(a),lt(),A()})})}function W(e,t){let n=document.querySelector(`[${e}="${t}"]`);return n?.click(),n!==null}function Or(e){let t=!1;return e.radius!==void 0&&(t=W(D.radius,String(e.radius))||t),e.day&&(t=W(D.day,e.day)||t),e.oneSeatRestricted!==void 0&&W(D.oneSeatDay,e.oneSeatRestricted?"selected":"any"),e.weight&&(rt=e.weight),e.surfaceUnit&&(de=e.surfaceUnit),e.placeFill&&W(D.placeFill,e.placeFill),e.dest&&("key"in e.dest?W(D.dest,e.dest.key):Le(e.dest)),e.selection&&In(i,e.selection),e.view&&W(D.view,e.view),e.at&&dn(e.at.lat,e.at.lon),e.place&&tt(e.place),t}function A(){let e={view:y,day:L(),radius:P,oneSeatRestricted:ue,weight:rt,surfaceUnit:de,dest:$,at:f,camera:_o,place:ot,placeFill:M,selection:Bn()},t=ho(e);history.replaceState(null,"",($e?wo(t):t)+location.hash),it(t)}function it(e=So(location.search)){if(!$e)return;let t=c("embed-link");t.href=`${location.pathname}${e}${location.hash}`;let n=f?ie?ye(ie):"this point":null;t.querySelector(".el-action").textContent=Lo(n)}function lt(){c("statebar").innerHTML=po({view:y,day:L(),radius:P,oneSeatRestricted:ue,destination:ke()}),Rr()}function on(e){c("legend-box").classList.toggle("collapsed",e);let t=c("legend-collapse");t.textContent=e?"+":"\u2013",t.title=e?"Show the key":"Collapse the key",t.setAttribute("aria-expanded",String(!e))}function Dr(){let e=t=>{c("app").classList.toggle("controls-open",t),c("controls-toggle").setAttribute("aria-expanded",String(t))};c("controls-toggle").addEventListener("click",()=>{e(!c("app").classList.contains("controls-open"))}),c("controls-scrim").addEventListener("click",()=>e(!1)),document.addEventListener("keydown",t=>{t.key==="Escape"&&e(!1)})}function Rr(){c("controls-toggle").firstChild?.remove(),c("controls-toggle").prepend(document.createTextNode(uo(y)))}function Tr(){let e=c("app").classList.toggle("side-collapsed"),t=c("side-toggle");t.textContent=e?"\u203A":"\u2039",t.title=e?"Show the panel":"Hide the panel",t.setAttribute("aria-expanded",String(!e)),i.resize()}function b(){Er()}function Er(){if(c("legend-reset").classList.toggle("hidden",At()||Ut()||Yt()||ve()),Yt()){c("legend").innerHTML=za(Ye());return}if(ve()){c("legend").innerHTML=io({selected:qa(),fill:M,day:L(),boundaries:Qt(),unchanged:Xa()});return}if(At()){let n=Ue();n&&ka(c("legend"),n);return}if(Ut()){let n=ne();if(!n)return;let a=i.getBounds();xa(c("legend"),n,{west:a.getWest(),south:a.getSouth(),east:a.getEast(),north:a.getNorth()});return}let e=St();if(!e)return;let t=i.getBounds();Pa(c("legend"),{layer:e,day:L(),bounds:{west:t.getWest(),south:t.getSouth(),east:t.getEast(),north:t.getNorth()},weight:rt,surface:Tt()?Ne():null,unit:de,population:Ae(),selection:Hn()})}async function Cr(e){if(e&&!Ne()){c("legend").classList.add("loading");try{await Et(i,P,L())}finally{c("legend").classList.remove("loading")}}oa(i,e),e&&de==="people"&&await Ro(),b()}async function Ro(){if(!Ae()){c("legend").classList.add("loading");try{await Ft(P)}finally{c("legend").classList.remove("loading")}}}async function Mr(e){e==="people"&&Tt()&&await Ro(),b()}async function Fr(e){if(e&&!Ue()){c("legend").classList.add("loading");try{await Ht(i,L())}finally{c("legend").classList.remove("loading")}}da(i,e),b()}async function Nr(e){if(e&&(!Xt()||!Qt())){c("legend").classList.add("loading");try{await Promise.all([to(),no(i)])}finally{c("legend").classList.remove("loading")}}oo(i,e),e&&Xe(i,M,L()),e&&F(),b()}async function tt(e){ot=await un(()=>ao(i,e))?e:null,y==="places"&&(F(),ot&&document.querySelector(`[data-select-place="${CSS.escape(e)}"]`)?.scrollIntoView({block:"nearest"})),b(),A()}function Ar(e){document.querySelectorAll("[data-radius]").forEach(t=>{t.disabled=!e})}function F({scrollToTop:e=!1}={}){if(e&&(c("panel").scrollTop=0),it(),y==="places"){c("panel").innerHTML=so(Xt()??[],Oo,ot,M);return}if(!ie){y==="oneseat"?c("panel").innerHTML=Fn(ke()):Rn(c("panel"));return}if(y==="oneseat"){let t=Mn(ie,$,L());if(t){c("panel").innerHTML=t;return}}Cn(ie)}function Hr(e,t=!1){if(ja(i,e),b(),!e){t&&(f?le(f.lat,f.lon):F());return}Ye()&&f?c("panel").innerHTML=Wt(Ye(),ke()):c("panel").innerHTML=Ja(ke())}async function ln(e,t){let n=++q;f={lat:e,lon:t},A(),Co(e,t);let a=Eo(),o=d(ke());if(!a){c("panel").innerHTML=`<div class="empty"><h2>No destination yet</h2>
    ...
    <p class="muted">Routing both networks from this point to
       ...
       <p class="muted">${s.message}</p></div>`}}function sn(){c("day-controls").classList.toggle("hidden",!va(y,ue,M))}function cn(){return ba(ue,L())}async function Br(e){e&&!ne()&&await un(()=>It(i,P,$,cn())),wa(i,e),b()}async function nt(){await un(()=>It(i,P,$,cn())),b()}async function un(e){c("legend").classList.add("loading");try{return await e()}finally{c("legend").classList.remove("loading")}}function Le(e){if($=e,at(!1),jr(),To(),lt(),A(),y==="journey"){f&&ln(f.lat,f.lon),b();return}f?le(f.lat,f.lon):F({scrollToTop:!0}),nt()}function To(){let e=Eo();if(!(e!==null&&(y==="journey"||y==="oneseat"&&"lat"in $))){V?.remove(),V=null;return}V?V.setLngLat([e.lon,e.lat]).addTo(i):(V=new maplibregl.Marker({color:jt,draggable:!0}).setLngLat([e.lon,e.lat]).addTo(i),V.on("dragend",()=>{let n=V.getLngLat();Le({lat:n.lat,lon:n.lng})}))}function jr(){let e=fa($);document.querySelectorAll("[data-dest]").forEach(t=>{t.classList.toggle("active",t.dataset.dest===e)})}function Eo(){if("lat"in $)return{lat:$.lat,lon:$.lon};let e=$.key,t=rn.find(n=>n.key===e);return t?{lat:t.lat,lon:t.lon}:null}function ke(){if("lat"in $)return`${$.lat.toFixed(4)}, ${$.lon.toFixed(4)}`;let e=$.key;return rn.find(t=>t.key===e)?.name??e}function at(e){Po=e,i.getCanvas().style.cursor=e?"crosshair":"",document.querySelectorAll('[data-dest="pin"]').forEach(t=>{t.classList.toggle("armed",e),t.textContent=e?"click the map\u2026":"Pick a point"})}async function le(e,t){let n=++q;f={lat:e,lon:t},A(),c("panel").classList.add("loading"),Co(e,t);try{let a="lat"in $?`&dest_lat=${$.lat.toFixed(6)}&dest_lon=${$.lon.toFixed(6)}`:"",o=await S(`/api/place?lat=${e.toFixed(6)}&lon=${t.toFixed(6)}&radius=${P}${a}&oneseat_day=${cn()}`);if(n!==q)return;gn(i,e,t,P,o.current.stops,o.proposed.stops),Ur(),ie=o,F({scrollToTop:!0})}catch(a){if(n!==q)return;c("panel").innerHTML=`<div class="empty"><h2>No answer for that point</h2>
       <p class="muted">${a.message}</p></div>`}finally{n===q&&c("panel").classList.remove("loading")}}function Ur(){c("pin-key").innerHTML=_a(P),c("pin-key").classList.remove("hidden")}function Co(e,t){we?we.setLngLat([t,e]):(we=new maplibregl.Marker({color:_r,draggable:!0}).setLngLat([t,e]).addTo(i),we.on("dragend",()=>{let n=we.getLngLat();dn(n.lat,n.lng)}))}var et=14;function st(){return y==="dots"||y==="both"}function xo(e){N=e&&st(),N?i.dragPan.disable():i.dragPan.enable(),i.getCanvas().style.cursor=N?"none":"",N||Mo(),ce()}function ce(){let e=c("legend-select");e.classList.toggle("hidden",!st()),e.setAttribute("aria-pressed",String(N)),e.textContent=N?"Selecting":"Select stops",c("legend-clear").classList.toggle("hidden",!st()||!jn())}function Ir(e,t){let n=c("brush");n.style.left=`${e}px`,n.style.top=`${t}px`,n.hidden=!N}function Mo(){c("brush").hidden=!0}function Jr(){let e=c("brush");e.style.width=`${et*2}px`,e.style.height=`${et*2}px`;let t=!1,n=!1,a=!1,o=()=>{a||(a=!0,requestAnimationFrame(()=>{a=!1,ce(),b()}))},s=()=>{N&&(t=!0,n=!1)},r=u=>{if(Ir(u.point.x,u.point.y),!t)return;n=!0,$t(i,xt(i,u.point.x,u.point.y,et))&&o()},l=u=>{if(t){if(t=!1,!n){let[p]=xt(i,u.point.x,u.point.y,et);p&&Un(i,p)}ce(),b(),A()}};i.on("mousedown",s),i.on("mousemove",r),i.on("mouseup",l),i.getCanvas().addEventListener("mouseleave",Mo),i.on("touchstart",s),i.on("touchmove",r),i.on("touchend",l)}function dn(e,t){if(Do.atLeast("half"),y==="journey"){ln(e,t);return}y!=="places"&&le(e,t)}async function zr(){try{rn=await S("/api/destinations"),lt()}catch{}}async function Gr(){try{let e=await S("/api/meta"),t=`today: ${e.feeds.current_feed_version||"current GTFS"} \xB7 proposed: ${e.feeds.proposed_feed_version||"proposed-network feed"}`;c("feedline").textContent=t,c("feedline-methods").textContent=t,c("caveats").innerHTML=e.caveats.map(n=>`<li id="caveat-${n.id}">${n.text}</li>`).join("")}catch{}}function Kr(e){c("methods").classList.add("open");let t=document.getElementById(`caveat-${e}`);t&&(t.scrollIntoView({block:"center"}),t.classList.remove("asked"),t.offsetWidth,t.classList.add("asked"))}c("methods-open").addEventListener("click",()=>c("methods").classList.add("open"));c("methods-close").addEventListener("click",()=>c("methods").classList.remove("open"));})();
