"use strict";(()=>{function i(e){let t=document.getElementById(e);if(!t)throw new Error(`missing element #${e}`);return t}async function L(e){let t=await fetch(e);if(!t.ok){let n=t.statusText;try{n=(await t.json()).detail??n}catch{}throw new Error(n)}return t.json()}function y(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function A(e){if(e==null)return"\u2014";let t=e%1440,n=Math.floor(t/60),o=Math.round(t%60),a=n<12?"am":"pm";return`${n%12===0?12:n%12}:${String(o).padStart(2,"0")}${a}`}function Re(e){return e>0?`+${e}`:String(e)}function ht(e,t){return e?`${t>=e?"+":""}${((t-e)/e*100).toFixed(1)}%`:t?"new":"\u2014"}var eo="#4aa3ff",to="#ffa23a";function no(e,t,n,o=96){let a=[],r=n/111320,c=n/(111320*Math.cos(e*Math.PI/180));for(let s=0;s<=o;s++){let d=s/o*2*Math.PI;a.push([t+c*Math.cos(d),e+r*Math.sin(d)])}return{type:"Feature",geometry:{type:"Polygon",coordinates:[a]},properties:{}}}function N(e){return{type:"FeatureCollection",features:e}}function ft(e,t){return e.map(n=>({type:"Feature",geometry:{type:"Point",coordinates:[n.lon,n.lat]},properties:{...n,side:t}}))}function gt(e){e.addSource("walk",{type:"geojson",data:N([])}),e.addSource("stops-now",{type:"geojson",data:N([])}),e.addSource("stops-prop",{type:"geojson",data:N([])}),e.addLayer({id:"walk-fill",type:"fill",source:"walk",paint:{"fill-color":"#8fb7ff","fill-opacity":.12}}),e.addLayer({id:"walk-line",type:"line",source:"walk",paint:{"line-color":"#8fb7ff","line-width":1.5,"line-dasharray":[2,2]}}),e.addLayer({id:"stops-prop-c",type:"circle",source:"stops-prop",paint:{"circle-radius":7,"circle-color":to,"circle-opacity":.85,"circle-stroke-width":1,"circle-stroke-color":"#3a2a10"}}),e.addLayer({id:"stops-now-c",type:"circle",source:"stops-now",paint:{"circle-radius":4,"circle-color":eo,"circle-stroke-width":1,"circle-stroke-color":"#0d2036"}});let t=new maplibregl.Popup({closeButton:!1,offset:10});for(let n of["stops-now-c","stops-prop-c"])e.on("mouseenter",n,()=>{e.getCanvas().style.cursor="pointer"}),e.on("mouseleave",n,()=>{e.getCanvas().style.cursor="",t.remove()}),e.on("mousemove",n,o=>{let a=o.features?.[0];if(!a)return;let r=a.properties;t.setLngLat(o.lngLat).setHTML(`<b>${r.name}</b><br>${r.side==="current"?"today":"proposed"}
    ...
    <div class="empty">
      ...
      </tr>`}).join("")}function R(e){return e.length?e.map(t=>`<span class="route">${y(t)}</span>`).join(" "):'<span class="muted">none</span>'}function Lt(e){return e.first==null?'<span class="muted">no service</span>':`${A(e.first)} \u2013 ${A(e.last)}`}function kt(e){let t=Object.values(e.headways).map(n=>n.median).filter(n=>n!=null);return t.length?Math.min(...t):null}var ro={here:"you are here",keeps:"keeps a one-seat ride",gains:"gains a one-seat ride",loses:"loses its one-seat ride",none:"no one-seat ride either way"},so={weekday:"a weekday",saturday:"a Saturday",sunday:"a Sunday"};function io(e,t="any"){return e.length?`
    <div class="oneseat">
      ...
      <div class="os-row">
        <div class="os-head">
          ...
          <span class="os-status ${y(o.status)}">${ro[o.status]??o.status}</span>
        </div>
      ...
      <p class="note">A one-seat ride means some single route serves both this
        spot and the destination. ${t==="any"?`Counted on any calendar, which is the published measure \u2014 no day
             type enters it.`:`Restricted to routes running on ${so[t]??t},
    ...
    <button class="os-other" data-goto-dest="${y(a.key)}">
      ...
      <span class="os-status ${y(a.status)}">${fo[a.status]}</span>
    </button>`).join("")}</div>
      <p class="note">Click one to measure the whole map to it instead.</p>
       ...
       on any calendar.`}function Dt(e,t,n){let o=po(e,t);if(!o)return"";let a=e.oneseat_day??"any",r=o.status==="here"?"":mo(o)+yo(o);return`
    <div class="place-head">
      <h2>One-seat ride to ${y(o.name)}</h2>
      <div class="muted">
        from ${y(U(e))} \xB7 ${e.lat.toFixed(5)}, ${e.lon.toFixed(5)} \xB7
        within ${e.radius} m
      </div>
    </div>

    <div class="os-verdict ${y(o.status)}">${lo[o.status]}</div>
    ...
    <span><i class="sw-prop"></i>stop proposed</span>`}function mn(e,t,n,o,a){let r=t.buckets.map(u=>u.key),c=Ct(t.points,t.days.indexOf(n),r,o.west,o.south,o.east,o.north),s=Co(t),d=s.reduce((u,p)=>u+c[p.key],0);e.innerHTML=`
    <div class="lg-head">
    ...
    <p class="lg-foot">Rides follow the street the bus drives; dashed sections
      ...
      panel answers at a stricter one too.</p>`}var at=" \xB7 ",rt={dots:"Locations",surface:"Surface",both:"Locations + surface",corridors:"Streets",oneseat:"One-seat ride",journey:"Travel time"},Cn=Object.keys(rt);function En(e){return rt[e]??e}var Io={weekday:"a weekday",saturday:"a Saturday",sunday:"a Sunday"},zo=["oneseat","journey"];function Ko(e){return e!=="journey"}function Uo(e){let t=[rt[e.view]??e.view];return zo.includes(e.view)&&(t[0]+=` to ${e.destination}`),t.push(e.view==="oneseat"&&!e.oneSeatRestricted?"any day":Io[e.day]),Ko(e.view)&&t.push(`${e.radius} m walk`),t.join(at)}function Mn(e){let[t,...n]=Uo(e).split(at);return`<b>${y(t)}</b>${n.map(o=>at+y(o)).join("")}`}var v={view:"view",day:"day",radius:"radius",oneSeatDay:"oneseatday",dest:"dest",at:"at",camera:"map"},ve={any:"any",selected:"selected"},Yo="pin",Pn=5;function Nn(e){try{return e.self!==e.top}catch{return!0}}function Fn(e){let t=new URLSearchParams;return t.set(v.view,e.view),t.set(v.day,e.day),t.set(v.radius,String(e.radius)),t.set(v.oneSeatDay,e.oneSeatRestricted?ve.selected:ve.any),t.set(v.dest,"key"in e.dest?e.dest.key:st(e.dest)),e.at&&t.set(v.at,st(e.at)),e.camera&&t.set(v.camera,`${st(e.camera)},${e.camera.zoom.toFixed(2)}`),`?${t}`}function Hn(e){let t=new URLSearchParams(e),n={},o=t.get(v.view);o&&Cn.includes(o)&&(n.view=o);let a=t.get(v.day);a&&k.includes(a)&&(n.day=a);let r=Number(t.get(v.radius));t.has(v.radius)&&Number.isFinite(r)&&r>0&&(n.radius=r);let c=t.get(v.oneSeatDay);c===ve.selected?n.oneSeatRestricted=!0:c===ve.any&&(n.oneSeatRestricted=!1);let s=t.get(v.dest);if(s&&s!==Yo){let p=An(s);p?n.dest=p:s.includes(",")||(n.dest={key:s})}let d=An(t.get(v.at));d&&(n.at=d);let u=Vo(t.get(v.camera));return u&&(n.camera=u),n}function st(e){return`${e.lat.toFixed(Pn)},${e.lon.toFixed(Pn)}`}function An(e){let t=jn(e,2);return t?{lat:t[0],lon:t[1]}:null}function Vo(e){let t=jn(e,3);return t?{lat:t[0],lon:t[1],zoom:t[2]}:null}function jn(e,t){if(!e)return null;let n=e.split(",").map(Number);return n.length!==t||!n.every(Number.isFinite)?null:n}var it="embed";var Wo=["1","true","yes"];function Jn(e){let t=new URLSearchParams(e).get(it);return t!==null&&Wo.includes(t.toLowerCase())}function Bn(e){let t=new URLSearchParams(e);return t.set(it,"1"),`?${t}`}function In(e){let t=new URLSearchParams(e);t.delete(it);let n=String(t);return n?`?${n}`:""}function zn(e){return(e?`Full answer for ${e}`:"Open the full map")+" \u2197"}var _=["peek","half","full"],Go=192,qo=.3,Xo=.55,Qo=.9,Zo=.6,ea=.45;function Se(e,t){return e==="peek"?Math.min(Go,t*qo):e==="half"?t*Xo:t*Qo}function ta(e,t,n=0){let o=_.map(r=>Math.abs(Se(r,t)-e)),a=o.indexOf(Math.min(...o));return Math.abs(n)>Zo&&(a=Math.max(0,Math.min(_.length-1,a+(n>0?1:-1)))),_[a]}function Kn(e){return _[(_.indexOf(e)+1)%_.length]}function na(e,t){return Math.min(e,t*ea)}function J(){return getComputedStyle(document.documentElement).getPropertyValue("--compact").trim()==="1"}function lt(e){let t=null,n=()=>{let o=J();o!==t&&(t=o,e(o))};return window.addEventListener("resize",n),n(),n}var oa=8,aa=400;function Un(e){let t=i("side"),n=i("sheet-handle"),o="peek",a=!1,r=0,c=0,s=0,d={y:0,t:0};function u(){return window.innerHeight}function p(m){t.style.height=`${m}px`,e.onMove(m,na(m,u()))}function g(m){o=m,t.dataset.snap=m,p(Se(m,u()))}n.addEventListener("pointerdown",m=>{J()&&(a=!0,r=m.clientY,c=t.getBoundingClientRect().height,s=m.timeStamp,d={y:m.clientY,t:m.timeStamp},t.classList.add("dragging"),n.setPointerCapture(m.pointerId))}),n.addEventListener("pointermove",m=>{if(!a)return;let yt=c+(r-m.clientY),ne=Se("peek",u()),Oe=Se("full",u());p(Math.max(ne,Math.min(Oe,yt))),d={y:m.clientY,t:m.timeStamp}});function O(m){if(!a)return;if(a=!1,t.classList.remove("dragging"),!(Math.abs(m.clientY-r)>oa)&&m.timeStamp-s<aa){g(Kn(o));return}let ne=m.timeStamp-d.t,Oe=ne>0?(d.y-m.clientY)/ne:0;g(ta(t.getBoundingClientRect().height,u(),Oe))}n.addEventListener("pointerup",O),n.addEventListener("pointercancel",O),n.addEventListener("keydown",m=>{m.key!=="Enter"&&m.key!==" "||(m.preventDefault(),J()&&g(Kn(o)))});let D=lt(e.onLayoutChange);function P(){if(D(),!J()){t.style.height="",t.removeAttribute("data-snap"),e.onMove(0,0);return}g(o)}return window.addEventListener("resize",P),P(),{at:()=>J()?o:"full",atLeast(m){J()&&_.indexOf(m)>_.indexOf(o)&&g(m)}}}var ra=[-79.9959,40.4406],sa=12,ia="#e2574c",x={radius:"data-radius",day:"data-day",oneSeatDay:"data-oneseat-day",view:"data-view",dest:"data-dest"},X=Hn(location.search),Z=Jn(location.search);Z&&i("app").classList.add("embed");var la={at:()=>"full",atLeast(){}},Vn=null,$=400,W=null,f=null,B=null,M=0,b={key:"downtown"},E=null,Wn=!1,K=!1,h="dots",Gn,dt=[],l=new maplibregl.Map({container:"map",style:"https://tiles.openfreemap.org/styles/positron",center:X.camera?[X.camera.lon,X.camera.lat]:ra,zoom:X.camera?.zoom??sa,cooperativeGestures:Nn(window),attributionControl:{compact:!0}});l.addControl(new maplibregl.NavigationControl,"top-right");l.on("load",()=>{gt(l),Mt(l),Ut(l,"change-dots"),qt(l,"change-dots"),on(l,"walk-fill"),xn(l),I(),l.on("click",t=>{if(Wn){Q({lat:t.lngLat.lat,lon:t.lngLat.lng});return}let n=["change-dots","oneseat-dots"].filter(r=>l.getLayoutProperty(r,"visibility")!=="none"),o=l.queryRenderedFeatures(t.point,{layers:n})[0],a=o?o.geometry.coordinates:[t.lngLat.lng,t.lngLat.lat];mt(a[1],a[0])});let e=new maplibregl.Popup({closeButton:!1,offset:8});l.on("mouseenter","change-dots",()=>{l.getCanvas().style.cursor="pointer"}),l.on("mouseleave","change-dots",()=>{l.getCanvas().style.cursor="",e.remove()}),l.on("mousemove","change-dots",t=>{let n=t.features?.[0],o=Ae();!n||!o||e.setLngLat(n.geometry.coordinates).setHTML(Nt(n.properties,w(),o.buckets)).addTo(l)}),l.on("mouseenter","oneseat-dots",()=>{l.getCanvas().style.cursor="pointer"}),l.on("mouseleave","oneseat-dots",()=>{l.getCanvas().style.cursor="",e.remove()}),l.on("mousemove","oneseat-dots",t=>{let n=t.features?.[0],o=j();!n||!o||e.setLngLat(n.geometry.coordinates).setHTML(cn(n.properties,o)).addTo(l)}),l.on("moveend",()=>{let t=l.getCenter();Vn={lat:t.lat,lon:t.lng,zoom:l.getZoom()},S(),te()}),G(x.radius,t=>{$=Number(t.dataset.radius),Fe(l,$,w()).then(S),ce()&&Ie(l,$,w()).then(S),j()&&Le(),f&&z(f.lat,f.lon)}),G(x.day,t=>{let n=t.dataset.day;$t(n),h!=="journey"&&I(),He(l,n),ze(l,n),h==="journey"&&f&&ut(f.lat,f.lon),me()&&Xt(l,n).then(S),K&&j()&&(Le(),f&&z(f.lat,f.lon)),S()}),G(x.oneSeatDay,t=>{K=t.dataset.oneseatDay==="selected",Yn(),Le(),f&&z(f.lat,f.lon)}),G(x.view,t=>{let n=h;h=t.dataset.view,l.setLayoutProperty("change-dots","visibility",h==="dots"||h==="both"?"visible":"none"),ya(h==="surface"||h==="both"),ha(h==="corridors"),ba(h==="oneseat"),ga(h==="journey",n==="journey"),h!=="journey"&&n!=="journey"&&(h==="oneseat"||n==="oneseat")&&I({scrollToTop:!0}),fa(h!=="corridors"&&h!=="journey");let o=h==="oneseat"||h==="journey";i("dest-controls").classList.toggle("hidden",!o),i("oneseat-day-controls").classList.toggle("hidden",h!=="oneseat"),Yn(),o||ke(!1),Xn()}),G(x.dest,t=>{let n=t.dataset.dest;if(n==="pin"){ke(!0);return}ke(!1),Q({key:n})}),i("legend").addEventListener("click",t=>{let n=t.target.closest("[data-bucket]");n&&(Pt(l,n.dataset.bucket,w()),S())}),i("legend-reset").addEventListener("click",()=>{At(l,w()),S()}),i("legend-collapse").addEventListener("click",()=>{ct(!i("legend-box").classList.contains("collapsed"))}),i("panel").addEventListener("click",t=>{let n=t.target.closest("[data-goto-dest]");n&&Q({key:n.dataset.gotoDest})}),i("side-toggle").addEventListener("click",pa),Z&&lt(ct),Gn=Z?la:Un({onMove(t,n){document.documentElement.style.setProperty("--sheet-h",`${t}px`),l.setPadding({top:0,right:0,bottom:n,left:0})},onLayoutChange:ct}),da(),xe(),$e(),ca(X)||Fe(l,$,w()).then(S),La(),Sa()});function G(e,t){let n=`[${e}]`;document.querySelectorAll(n).forEach(o=>{o.addEventListener("click",()=>{document.querySelectorAll(n).forEach(a=>a.classList.toggle("active",a===o)),t(o),xe(),te()})})}function q(e,t){let n=document.querySelector(`[${e}="${t}"]`);return n?.click(),n!==null}function ca(e){let t=!1;return e.radius!==void 0&&(t=q(x.radius,String(e.radius))||t),e.day&&(t=q(x.day,e.day)||t),e.oneSeatRestricted!==void 0&&q(x.oneSeatDay,e.oneSeatRestricted?"selected":"any"),e.dest&&("key"in e.dest?q(x.dest,e.dest.key):Q(e.dest)),e.view&&q(x.view,e.view),e.at&&mt(e.at.lat,e.at.lon),t}function te(){let e={view:h,day:w(),radius:$,oneSeatRestricted:K,dest:b,at:f,camera:Vn},t=Fn(e);history.replaceState(null,"",(Z?Bn(t):t)+location.hash),$e(t)}function $e(e=In(location.search)){if(!Z)return;let t=i("embed-link");t.href=`${location.pathname}${e}${location.hash}`;let n=f?B?U(B):"this point":null;t.querySelector(".el-action").textContent=zn(n)}function xe(){i("statebar").innerHTML=Mn({view:h,day:w(),radius:$,oneSeatRestricted:K,destination:ee()}),ua()}function ct(e){i("legend-box").classList.toggle("collapsed",e);let t=i("legend-collapse");t.textContent=e?"+":"\u2013",t.title=e?"Show the key":"Collapse the key",t.setAttribute("aria-expanded",String(!e))}function da(){let e=t=>{i("app").classList.toggle("controls-open",t),i("controls-toggle").setAttribute("aria-expanded",String(t))};i("controls-toggle").addEventListener("click",()=>{e(!i("app").classList.contains("controls-open"))}),i("controls-scrim").addEventListener("click",()=>e(!1)),document.addEventListener("keydown",t=>{t.key==="Escape"&&e(!1)})}function ua(){i("controls-toggle").firstChild?.remove(),i("controls-toggle").prepend(document.createTextNode(En(h)))}function pa(){let e=i("app").classList.toggle("side-collapsed"),t=i("side-toggle");t.textContent=e?"\u203A":"\u2039",t.title=e?"Show the panel":"Hide the panel",t.setAttribute("aria-expanded",String(!e)),l.resize()}function S(){ma()}function ma(){if(i("legend-reset").classList.toggle("hidden",Ue()||Ge()||tt()),tt()){i("legend").innerHTML=Tn(we());return}if(Ue()){let n=me();n&&dn(i("legend"),n);return}if(Ge()){let n=j();if(!n)return;let o=l.getBounds();un(i("legend"),n,{west:o.getWest(),south:o.getSouth(),east:o.getEast(),north:o.getNorth()});return}let e=Ae();if(!e)return;let t=l.getBounds();mn(i("legend"),e,w(),{west:t.getWest(),south:t.getSouth(),east:t.getEast(),north:t.getNorth()},Jt()?ce():null)}async function ya(e){if(e&&!ce()){i("legend").classList.add("loading");try{await Ie(l,$,w())}finally{i("legend").classList.remove("loading")}}Yt(l,e),S()}async function ha(e){if(e&&!me()){i("legend").classList.add("loading");try{await Ye(l,w())}finally{i("legend").classList.remove("loading")}}Qt(l,e),S()}function fa(e){document.querySelectorAll("[data-radius]").forEach(t=>{t.disabled=!e})}function I({scrollToTop:e=!1}={}){if(e&&(i("panel").scrollTop=0),$e(),!B){h==="oneseat"?i("panel").innerHTML=Tt(ee()):xt(i("panel"));return}if(h==="oneseat"){let t=Dt(B,b,w());if(t){i("panel").innerHTML=t;return}}_t(B)}function ga(e,t=!1){if(On(l,e),S(),!e){t&&(f?z(f.lat,f.lon):I());return}we()&&f?i("panel").innerHTML=ot(we(),ee()):i("panel").innerHTML=Dn(ee())}async function ut(e,t){let n=++M;f={lat:e,lon:t},te(),Zn(e,t);let o=Qn(),a=y(ee());if(!o){i("panel").innerHTML=`<div class="empty"><h2>No destination yet</h2>
