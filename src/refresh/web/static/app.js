"use strict";(()=>{function u(e){let t=document.getElementById(e);if(!t)throw new Error(`missing element #${e}`);return t}async function b(e){let t=await fetch(e);if(!t.ok){let n=t.statusText;try{n=(await t.json()).detail??n}catch{}throw new Error(n)}return t.json()}function p(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function O(e){if(e==null)return"\u2014";let t=e%1440,n=Math.floor(t/60),o=Math.round(t%60),r=n<12?"am":"pm";return`${n%12===0?12:n%12}:${String(o).padStart(2,"0")}${r}`}function re(e){return e>0?`+${e}`:String(e)}function He(e,t){return e?`${t>=e?"+":""}${((t-e)/e*100).toFixed(1)}%`:t?"new":"\u2014"}var Zt="#4aa3ff",en="#ffa23a";function tn(e,t,n,o=96){let r=[],a=n/111320,c=n/(111320*Math.cos(e*Math.PI/180));for(let s=0;s<=o;s++){let l=s/o*2*Math.PI;r.push([t+c*Math.cos(l),e+a*Math.sin(l)])}return{type:"Feature",geometry:{type:"Polygon",coordinates:[r]},properties:{}}}function R(e){return{type:"FeatureCollection",features:e}}function Be(e,t){return e.map(n=>({type:"Feature",geometry:{type:"Point",coordinates:[n.lon,n.lat]},properties:{...n,side:t}}))}function ze(e){e.addSource("walk",{type:"geojson",data:R([])}),e.addSource("stops-now",{type:"geojson",data:R([])}),e.addSource("stops-prop",{type:"geojson",data:R([])}),e.addLayer({id:"walk-fill",type:"fill",source:"walk",paint:{"fill-color":"#8fb7ff","fill-opacity":.12}}),e.addLayer({id:"walk-line",type:"line",source:"walk",paint:{"line-color":"#8fb7ff","line-width":1.5,"line-dasharray":[2,2]}}),e.addLayer({id:"stops-prop-c",type:"circle",source:"stops-prop",paint:{"circle-radius":7,"circle-color":en,"circle-opacity":.85,"circle-stroke-width":1,"circle-stroke-color":"#3a2a10"}}),e.addLayer({id:"stops-now-c",type:"circle",source:"stops-now",paint:{"circle-radius":4,"circle-color":Zt,"circle-stroke-width":1,"circle-stroke-color":"#0d2036"}});let t=new maplibregl.Popup({closeButton:!1,offset:10});for(let n of["stops-now-c","stops-prop-c"])e.on("mouseenter",n,()=>{e.getCanvas().style.cursor="pointer"}),e.on("mouseleave",n,()=>{e.getCanvas().style.cursor="",t.remove()}),e.on("mousemove",n,o=>{let r=o.features?.[0];if(!r)return;let a=r.properties;t.setLngLat(o.lngLat).setHTML(`<b>${a.name}</b><br>${a.side==="current"?"today":"proposed"}
    ...
    </div>`:""}function ue(e){le=e;let t=document.getElementById("panel"),n=e.current.days[D],o=e.proposed.days[D],r=o.trips-n.trips,a=r>0?"up":r<0?"down":"flat",c=e.place?.hood||e.place?.muni||"this location",s=Ue(n),l=Ue(o);t.innerHTML=`
    ...
    <div class="lg-foot">A piece of street either has a bus on it or it doesn't \u2014
      ...
      what you can still reach on foot.</div>`}function Ot(e,t,n){let o=t.statuses.map(l=>l.key),r=Lt(t.points,o,n.west,n.south,n.east,n.north),a=l=>t.statuses.find(d=>d.key===l)?.label??l,c=X.reduce((l,d)=>l+(r[d]??0),0),s=Re(t);e.innerHTML=`
    <div class="lg-head">
    ...
    <div class="lg-foot">Can a rider reach ${p(s)} without transferring?
      ...
      South Hills losing rides the Blue Line still runs.</div>`}function Rt(e,t,n,o,r){let a=t.buckets.map(d=>d.key),c=qe(t.points,t.days.indexOf(n),a,o.west,o.south,o.east,o.north),s=vn(t),l=s.reduce((d,g)=>d+c[g.key],0);e.innerHTML=`
    <div class="lg-head">
      <b>${l.toLocaleString()}</b> locations in view
      <span class="muted">\xB7 ${wn[n]} \xB7 ${t.radius} m walk</span>
    </div>
    ${s.map(d=>`
      <button class="lg-row ${me(d.key)?"off":""}" data-bucket="${p(d.key)}"
              aria-pressed="${!me(d.key)}">
        <i style="background:${B[d.key]?.color??"#666"}"></i>
    ...
    <div class="lg-foot">Buses per day within the walk radius, both directions.
      Counts are locations, not riders.</div>`}var De="#4aa3ff",jt="#ffa23a",Ce="headline",Q="journey",Jt="journey-rides",Ft="journey-walks",xn=[Jt,Ft],Nt=null,At=!1;function ee(){return Nt}function Me(){return At}function _n(e,t){let n=e.radii[t],o=[];for(let r of["current","proposed"]){let a=n[r].itinerary;if(a)for(let c of a.legs){let s=c.from??e.origin,l=c.to??e.destination;o.push({type:"Feature",geometry:{type:"LineString",coordinates:[[s.lon,s.lat],[l.lon,l.lat]]},properties:{side:r,kind:c.kind,route:c.route}})}}return{type:"FeatureCollection",features:o}}function Dt(){return["match",["get","side"],"current",De,"proposed",jt,De]}function Ct(e){let t=(n,o)=>["match",["get","side"],"proposed",o*e,n*e];return["interpolate",["linear"],["zoom"],9,t(3.5,2),14,t(7,4)]}function Ht(e,t){e.addSource(Q,{type:"geojson",data:{type:"FeatureCollection",features:[]}}),e.addLayer({id:Jt,type:"line",source:Q,filter:["==",["get","kind"],"ride"],layout:{visibility:"none","line-cap":"round","line-join":"round"},paint:{"line-color":Dt(),"line-width":Ct(1),"line-opacity":.85}},t),e.addLayer({id:Ft,type:"line",source:Q,filter:["==",["get","kind"],"walk"],layout:{visibility:"none","line-cap":"butt","line-join":"round"},paint:{"line-color":Dt(),"line-width":Ct(.6),"line-opacity":.8,"line-dasharray":[1.5,1.5]}},t)}function Bt(e,t){At=t;for(let n of xn)e.setLayoutProperty(n,"visibility",t?"visible":"none")}function Te(e,t){Nt=t;let n=t?_n(t,Ce):{type:"FeatureCollection",features:[]};e.getSource(Q).setData(n)}function zt(e,t,n){return`/api/journey?lat=${e.lat.toFixed(6)}&lon=${e.lon.toFixed(6)}&dest_lat=${t.lat.toFixed(6)}&dest_lon=${t.lon.toFixed(6)}&day=${n}`}var Mt=e=>`${e.toFixed(1)} min`;function Kt(e){return e==null?"\u2014":e===0?"no change":e>0?`${Mt(e)} slower`:`${Mt(-e)} faster`}function Tt(e,t){return e?e.name?p(e.name):`stop ${p(e.stop_id)}`:t}function On(e,t){let n=Math.round(e.arrive-e.depart);if(e.kind==="walk"){let o=Tt(e.to,"the destination");return`<div class="jl"><span class="jl-what">walk ${n} min</span>
            <span class="muted">to ${o}</span></div>`}return`<div class="jl"><span class="jl-what">ride
          <span class="route">${p(e.route??"?")}</span> ${n} min</span>
          <span class="muted">to ${Tt(e.to,"the destination")}</span></div>`}function Et(e,t){let n=[],o=null;for(let r of e.legs){let a=o?Math.round(r.depart-o.arrive):0;a>0&&n.push(`<div class="jl jl-wait"><span class="jl-what">wait ${a} min</span></div>`),n.push(On(r,t)),o=r}return n.join("")}var Rn={no_origin_coverage:"No bus stops within a walk of this point on one or both networks, so there is no trip to time from here. That is a coverage answer rather than a travel-time one \u2014 the Locations and Surface views are where it is measured.",no_dest_coverage:"No bus stops within a walk of the destination on one or both networks, so there is nothing to arrive at. That is a coverage answer rather than a travel-time one \u2014 the Locations and Surface views are where it is measured.",no_journey:"Both ends have buses, but no trip connects them inside this window on one or both networks \u2014 within the transfer walk below, and with a change of bus allowed."};function Z(e){return e.median_min==null?"\u2014":e.median_min.toFixed(1)}function Dn(e){return`
    ...
    </table>`}function Cn(e){let t=e.radii.strict,n=t.transfer_walk_m,o=e.sign_flips?`<p class="js-flip"><b>These two disagree about which network is
    ...
    every minute of the window, so half of them are worse.</p>`}function Ee(e,t){let n=e.radii[Ce],o=n.change_min==null?"flat":n.change_min>0?"down":n.change_min<0?"up":"flat",r=`
    <div class="place-head">
    ...
    <div class="empty">
      ...
      <p class="muted">It takes a moment \u2014 both networks are routed from
         ...
         be assumed.</p>
    </div>`}function Gt(e){let t=e?e.radii[Ce].transfer_walk_m:400;return`
    ...
    <p class="lg-foot">Dashed sections are walks. Straight lines between stops,
      ...
      stricter one too.</p>`}var Mn=[-79.9959,40.4406],Tn="#e2574c",S=400,F=null,h=null,_=0,y={key:"downtown"},x=null,Vt=!1,m="dots",Je=[],i=new maplibregl.Map({container:"map",style:"https://tiles.openfreemap.org/styles/positron",center:Mn,zoom:12});i.addControl(new maplibregl.NavigationControl,"top-right");i.on("load",()=>{ze(i),Qe(i),ut(i,"change-dots"),ft(i,"change-dots"),St(i,"walk-fill"),Ht(i),ce(u("panel")),i.on("click",t=>{if(Vt){Pe({lat:t.lngLat.lat,lon:t.lngLat.lng});return}let n=["change-dots","oneseat-dots"].filter(a=>i.getLayoutProperty(a,"visibility")!=="none"),o=i.queryRenderedFeatures(t.point,{layers:n})[0],r=o?o.geometry.coordinates:[t.lngLat.lng,t.lngLat.lat];Qt(r[1],r[0])});let e=new maplibregl.Popup({closeButton:!1,offset:8});i.on("mouseenter","change-dots",()=>{i.getCanvas().style.cursor="pointer"}),i.on("mouseleave","change-dots",()=>{i.getCanvas().style.cursor="",e.remove()}),i.on("mousemove","change-dots",t=>{let n=t.features?.[0],o=pe();!n||!o||e.setLngLat(n.geometry.coordinates).setHTML(tt(n.properties,v(),o.buckets)).addTo(i)}),i.on("mouseenter","oneseat-dots",()=>{i.getCanvas().style.cursor="pointer"}),i.on("mouseleave","oneseat-dots",()=>{i.getCanvas().style.cursor="",e.remove()}),i.on("mousemove","oneseat-dots",t=>{let n=t.features?.[0],o=J();!n||!o||e.setLngLat(n.geometry.coordinates).setHTML(xt(n.properties,o)).addTo(i)}),i.on("moveend",f),te("[data-radius]",t=>{S=Number(t.dataset.radius),ye(i,S,v()).then(f),G()&&we(i,S,v()).then(f),J()&&Yt(),h&&Ne(h.lat,h.lon)}),te("[data-day]",t=>{let n=t.dataset.day;We(n),fe(i,n),ve(i,n),m==="journey"&&h&&Fe(h.lat,h.lon),W()&&ht(i,n).then(f),f()}),te("[data-view]",t=>{let n=m;m=t.dataset.view,i.setLayoutProperty("change-dots","visibility",m==="dots"||m==="both"?"visible":"none"),En(m==="surface"||m==="both"),Pn(m==="corridors"),Fn(m==="oneseat"),Jn(m==="journey",n==="journey"),jn(m!=="corridors"&&m!=="journey");let o=m==="oneseat"||m==="journey";u("dest-controls").classList.toggle("hidden",!o),o||ne(!1),Wt()}),te("[data-dest]",t=>{let n=t.dataset.dest;if(n==="pin"){ne(!0);return}ne(!1),Pe({key:n})}),u("legend").addEventListener("click",t=>{let n=t.target.closest("[data-bucket]");n&&(Ze(i,n.dataset.bucket,v()),f())}),u("legend-reset").addEventListener("click",()=>{et(i,v()),f()}),ye(i,S,v()).then(f),Hn(),An()});function te(e,t){document.querySelectorAll(e).forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll(e).forEach(o=>o.classList.toggle("active",o===n)),t(n)})})}function f(){if(u("legend-reset").classList.toggle("hidden",Se()||_e()||Me()),Me()){u("legend").innerHTML=Gt(ee());return}if(Se()){let n=W();n&&_t(u("legend"),n);return}if(_e()){let n=J();if(!n)return;let o=i.getBounds();Ot(u("legend"),n,{west:o.getWest(),south:o.getSouth(),east:o.getEast(),north:o.getNorth()});return}let e=pe();if(!e)return;let t=i.getBounds();Rt(u("legend"),e,v(),{west:t.getWest(),south:t.getSouth(),east:t.getEast(),north:t.getNorth()},at()?G():null)}async function En(e){if(e&&!G()){u("legend").classList.add("loading");try{await we(i,S,v())}finally{u("legend").classList.remove("loading")}}dt(i,e),f()}async function Pn(e){if(e&&!W()){u("legend").classList.add("loading");try{await ke(i,v())}finally{u("legend").classList.remove("loading")}}bt(i,e),f()}function jn(e){document.querySelectorAll("[data-radius]").forEach(t=>{t.disabled=!e})}function Jn(e,t=!1){if(Bt(i,e),f(),!e){t&&(h?Ne(h.lat,h.lon):ce(u("panel")));return}ee()&&h?u("panel").innerHTML=Ee(ee(),je()):u("panel").innerHTML=It(je())}async function Fe(e,t){let n=++_;h={lat:e,lon:t},Xt(e,t);let o=qt(),r=p(je());if(!o){u("panel").innerHTML=`<div class="empty"><h2>No destination yet</h2>
