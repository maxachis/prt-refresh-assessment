...
| [The map hit-tests itself nineteen times per mouse move](the-map-hit-tests-itself-nineteen-times-per-mouse-move.md) | open, not fixed — 19 `queryRenderedFeatures` per pointer move, four of six layers not even in view; found while chasing a frame rate that turned out to be a GPU-less VM, so it is a defect on its own merits rather than that complaint's cause |
