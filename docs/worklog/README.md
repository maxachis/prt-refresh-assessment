...
| [Address search needs a geocoder, and the access log must not see the query](address-search-needs-a-geocoder-and-the-log-must-not-see-the-query.md) | open, decision owed — source found (county address points on WPRDC), licence unstated; search is a POST so the typed text never reaches the log |
