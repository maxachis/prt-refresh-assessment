...
| [The map hit-tests itself nineteen times per mouse move](the-map-hit-tests-itself-nineteen-times-per-mouse-move.md) | open, not fixed — 19 `queryRenderedFeatures` per pointer move, four of six layers not even in view; 2.5–3.5 ms of main-thread JS per frame of a drag; raised by Max as a frame-rate complaint |
