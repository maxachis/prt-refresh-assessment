...
| [A pin mark hides the dot underneath it](the-pin-mark-hides-the-dot-underneath-it.md) | fixed, awaiting close — one kerb, one reading: every dot now names its pole and says where the plan stands it, pin or no pin, at 65 KB gzipped; raised by Max |
...
| [A link carrying a day opens with no dots](a-link-carrying-a-day-opens-with-no-dots.md) | fixed, awaiting close — every link the app writes carries `day=`, and that press claimed a fetch it never made; the opening fetch is unconditional now, and no unit test can reach it |
| [The dot hover reported a district, not a stop](the-dot-hover-reported-a-district-not-a-stop.md) | fixed, awaiting close — a downtown dot said 1,591 buses a weekday; hover, colour and key are all its own kerb now, and the key's weekday "loses all service" boardings read 8.9% against the published 0.8% at a location; raised by Max |
| [Two deploys at once collide on the database build](two-deploys-at-once-collide-on-the-database-build.md) | open, decision owed — two sessions ran provision.sh together on 2026-09-11, one hit "database is locked", the other left an orphaned build; no lock on the box |
| [provision.sh reports "Deployed" when the box did nothing](provision-reports-deployed-when-the-box-did-nothing.md) | open, not fixed — the remote no-op branch exits 0 and the local banner says Deployed; the no-op test cannot tell a finished deploy from a failed one |
| [Address search needs a geocoder, and the access log must not see the query](address-search-needs-a-geocoder-and-the-log-must-not-see-the-query.md) | fixed, awaiting close — built 2026-09-17 on Max's decision to ship on the county's "licence not specified" |
| [Nothing on the map can be asked about one time of day](nothing-on-the-map-can-be-asked-about-one-time-of-day.md) | fixed for the dots and surface, awaiting close — the toolbar's *time* control, built 2026-09-17; travel time, streets and route cards still open, three decisions owed; raised by Max for PPT readers |
