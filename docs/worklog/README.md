...
| [The map hit-tests itself nineteen times per mouse move](the-map-hit-tests-itself-nineteen-times-per-mouse-move.md) | fixed, awaiting close — 19 hit tests per pointer move became 1, and the canvas pixel count was capped for software renderers; the frame rate that prompted it was a GPU-less VM |
