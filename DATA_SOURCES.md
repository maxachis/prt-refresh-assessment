...
| [Allegheny County Municipal Boundaries](https://data.wprdc.org/dataset/allegheny-county-municipal-boundaries) | GeoJSON | 130 municipal boundaries, from the county's own GIS. The authority for which municipality a point is in. `NAME`+`TYPE`, whose `LABEL` already reads the way PRT's `MUNI` spells a place. |
| [Pittsburgh Neighborhoods](https://data.wprdc.org/dataset/neighborhoods2) | Esri REST (GeoJSON) | The city's own 90 neighbourhood boundaries. Fetched from the Esri service listed on the WPRDC package because **WPRDC's own GeoJSON resource returns HTTP 500** (checked 2026-08-31); if it starts working it is the better citation. |
...
| [Overpass API](https://overpass-api.de/api/interpreter) (OpenStreetMap) | JSON, no key | **The pedestrian network.** Every walkable `highway` way in Allegheny County's bounding box — 1.0M nodes, 1.09M segments, 25,975 km — minus motorways, trunk roads and anything tagged `foot=no`/`access=private`. `highway=steps` is deliberately kept: Pittsburgh's public stairways are the actual route on the slopes. Cached gzipped at `data/raw/osm/allegheny_walk.json.gz` (16 MB) and **committed**, because OSM changes daily and a re-fetch would restate published travel times. Fetched by `ingest_osm_walk.py`; routed by `src/refresh/walking.py`. |
...
| [Allegheny County Addressing Address Points](https://data.wprdc.org/dataset/allegheny-county-addressing-address-points2) | CKAN DataStore SQL/dump | Every address point in the county, harvested weekly from the county's own GIS. Resource `ddc46dc5-65bf-42d2-b67d-dd45954b25da`, 653,615 rows, checked 2026-09-17. The web app's search box (`ingest_addresses.py`) fetches 13 columns via `?fields=` — `address_id, parent_id, status, addr_num, addr_num_suffix, st_prefix, st_name, st_type, unit, municipality, zip_code, latitude, longitude` — filters to `status = ACTIVE` and `parent_id = 0` (531,809 active building roots), and dedupes multi-building addresses to the lowest `address_id`. **Licence: `license_id: notspecified`** on the WPRDC page, and PASDA's landing page is silent too; **Max decided 2026-09-17 to ship on "not specified"** — see `docs/worklog/address-search-needs-a-geocoder-and-the-log-must-not-see-the-query.md`. |
