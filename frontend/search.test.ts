...
import {
  searchRequest, stopTag, routeTag, rows, groupOrder, nextHighlight, resultsHTML,
  easeTarget, NETWORK_TAG, SEARCH_LIMIT,
} from './search';
import { SearchResponse, SearchPlace, SearchStop, SearchRoute } from './types';
...
const response = (over: Partial<SearchResponse> = {}): SearchResponse => ({
  q: 'car', places: [place()], stops: [stop()], routes: [route()], ...over,
});
...
describe('rows', () => {
  it('runs places, then stops, then routes, in the order the headings show', () => {
    const r = rows(response());
    expect(r.map((x) => x.kind)).toEqual(['place', 'stop', 'route']);
  });

  it('is empty when nothing was found', () => {
    expect(rows(response({ places: [], stops: [], routes: [] }))).toEqual([]);
  });

  it('leads with routes when the query starts with a digit', () => {
    ...
    expect(groupOrder('61')).toEqual(['routes', 'places', 'stops']);
    expect(groupOrder('p1')).toEqual(['places', 'stops', 'routes']);
  });
});
...
describe('resultsHTML', () => {
  it('groups under three headings and shows each row\'s name and tag', () => {
    const html = resultsHTML(response(), null);
  ...
  it('says nothing was found when all three are empty', () => {
    const html = resultsHTML(response({ places: [], stops: [], routes: [] }), null);
